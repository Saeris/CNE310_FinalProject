    ...
    if request.method == "POST":
        ...
        with sqlite3.connect('database.db') as conn:
            ...
            if (password == old_password):
                try:
                    cur.execute(
                        "UPDATE users SET password = ? WHERE userId = ?", (new_password, user_id))
                    conn.commit()
...
def add_to_cart():
    if 'email' not in session:
        ...
        with sqlite3.connect('database.db') as conn:
            ...
            try:
                cur.execute(
                    "INSERT INTO kart (userId, productId) VALUES (?, ?)", (user_id, product_id))
                conn.commit()
                msg = "Added successfully"
            except:
                conn.rollback()
    ...
    with sqlite3.connect('database.db') as conn:
        ...
        try:
            cur.execute("DELETE FROM kart WHERE user_id = " +
                        str(user_id) + " AND productId = " + str(product_id))
            conn.commit()
...
def payment():
    ...
    for row in products:
        ...
        print(row)
        cur.execute(
            "INSERT INTO Orders (userId, productId) VALUES (?, ?)", (user_id, row[0]))
        ...
        with sqlite3.connect('database.db') as con:
            try:
                ...
                cur.execute('INSERT INTO users (password, email, firstName, lastName, address1, address2, zipcode, city, state, country, phone) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', (hashlib.md5(
                    password.encode()).hexdigest(), email, first_name, last_name, address1, address2, zipcode, city, state, country, phone))
